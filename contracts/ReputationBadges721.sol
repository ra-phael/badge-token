...
contract ReputationBadges721 is IERC721, ERC721NonTransferrable {
    ...
    event Minted (uint256 indexed providerId, address indexed to, uint256 indexed tokenId);
	...
	mapping(address => mapping(uint256 => uint256)) private _ownedTokenByProvider;
	...
	function getTokenByAddressAndProvider(address owner, uint256 providerId) public view returns (uint256) {
    ...
    function _mint(address to, uint256 tokenId, uint256 providerId) internal {
        require(to != address(0), "ERC721: mint to the zero address");
        require(!_exists(tokenId), "ERC721: token already minted");
        require(_ownedTokenByProvider[to][providerId] == 0, "Address already has a token for this provider");
